...
def index():
    if request.method == 'POST':
        try:
            ...
            try:
                uri = "mongodb+srv://admin:"+urllib.parse.quote('9912439639Aa@')+"@cluster0.at8clbw.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"

                client = MongoClient(uri)
                db=client['web_scrapper']
                collection=db['flipkart_scrapper']
                collection.insert_many(reviews)
                logging.info('sucessfully data is inserter')
            except Exception as e:
